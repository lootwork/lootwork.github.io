...
def tg_post(jobs):
    # Ручной запуск нужен, чтобы проверить правки, а не чтобы вывалить в канал
    # восемь постов. Публикуем только когда явно разрешено.
    if os.environ.get("TG_POST", "1") != "1":
        print("Telegram: постинг выключен для этого запуска")
        return
